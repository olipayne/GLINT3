...
import logging
from utils import common, GlintArgumentParser
...
class GlintParser(ModuleParser):
    def __init__(self, parser):
        ...
        optional.add_argument(
            "-h", "--h", "--help", action="help", help="Prints this help"
        )  # add help here so it will be under same group with all other optional argument
        optional.add_argument(
            "--out", type=str, help="Changes the prefix of the output file "
        )

        def loglevel_value(val):
            # val = int(val)
            if val.lower() not in list(configurelogging.OPTIONAL_LEVELS.keys()):
                common.terminate(
                    "Log level is not valid. should be one of %s"
                    % str(list(configurelogging.OPTIONAL_LEVELS.keys()))
                )
        ...
        modules.add_argument(
            "--refactor",
            action="store_true",
            help="Runs the ReFACTor algorithm for capturing cell type composition",
        )
        modules.add_argument("--ewas", action="store_true", help="Runs EWAS")
        modules.add_argument(
            "--impute",
            action="store_true",
            help="Imputes methylation levels from genotypes",
        )
        modules.add_argument(
            "--epi",
            action="store_true",
            help="Runs the EPISTRUCTURE algorithm for capturing population structure from methylation data",
        )
        modules.add_argument(
            "--plot", action="store_true", help="Allows to generate plots"
        )
        modules.add_argument(
            "--houseman",
            action="store_true",
            help="Runs the Houseman algorithm for estimating cell counts",
        )
...
class ModulesArgumentParsers(object):
    ...
    FUNCTIONALITY_ARGS = [
        ...
        "--impute",
        "--houseman",
    ]  # TODO find better way to hold arguments that cause some functionality. glint is not supposed to be aware of those args
    ...
    SOLE_ARGS = [
        "--epi"
    ]  # functilnality flags that cannot be specified with other functionaity flags

    DATA_PREPROCESSING_NOT_RELEVANT_FOR_REFACTOR = [
        "--include",
        "--exclude",
        "--minmean",
        "--maxmean",
    ...
    def __init__(self, user_args_selection):
        ...
        self.parser = GlintArgumentParser(
            prog=os.path.basename(sys.argv[0]), add_help=False
        )  # don't add help because it is added in 'optional group'
    ...
    def check_selected_args(self, optional_args):
        ...
        args_not_relevant_for_refactor = set(
            self.DATA_PREPROCESSING_NOT_RELEVANT_FOR_REFACTOR
        )
        all_func_args.update(sole_args)

        func_args_chosen = selected_args.intersection(all_func_args)
        args_not_relevant_for_refactor_chosen = selected_args.intersection(
            args_not_relevant_for_refactor
        )
        sole_args_chosen = selected_args.intersection(sole_args)

        if len(func_args_chosen) == 0:
            common.terminate(
                "Nothing to do with the data, select at least one argument from %s."
                % ", ".join(list(all_func_args))
            )

        # check that a sole argumet wasn't chosen with any other arg.
        elif (
            (len(func_args_chosen - data_func_args) > len(sole_args_chosen))
            and (len(sole_args_chosen) > 0)
        ) or (len(sole_args_chosen) > 1):
            common.terminate(
                "Options from %s cannot be specified with any arguments from %s in the same command. you chose %s."
                % (
                    str(list(sole_args)),
                    str(list(self.FUNCTIONALITY_ARGS)),
                    str(list(func_args_chosen)),
                )
            )
        ...
        if differ:
            ...
            if unrecognized_args:
                common.terminate(
                    "Unrecognized argument " + ", ".join(unrecognized_args)
                )
            if redundent_args:
                common.terminate(
                    "Selected redundent argument " + ", ".join(redundent_args)
                )
        ...
        if self.args.refactor and len(func_args_chosen) == 1:
            ...
            if len(not_relevant_atgs) != 0:
                logging.warning(
                    "Selected data management arguments which are not relevant for ReFACTor: %s."
                    % str(not_relevant_atgs)
                )
    ...
    def run(self):
        ...
        # plot runs without datafile but could receive datafile if run with ewas
        if (
            self.args.qqplot or args.manhattan
        ) and not self.args.ewas:  # if user asked to run plot without running EWAS test, run plot and quit
        ...
        if self.args.refactor:
            ...
            if self.args.gsave:
                logging.info(
                    "Adding the ReFACTor componetns to the covariates of the data..."
                )
            refactor_comp_names = self.meth_parser.module.add_covar_datas(
                self.refactor_parser.module.components, "rc"
            )  # add refactor components as covariate file
            ...
            if not self.args.gsave:
                logging.info(
                    "To use ReFACTor componetns as covariates run glint.py with --covarfile %s"
                    % self.refactor_parser.module.components_output_filename
                )
        ...
        if self.args.houseman:
            ...
            if self.args.gsave:
                logging.info(
                    "Adding the Houseman estimates to the covariates of the data."
                )
            houseman_comp_names = self.meth_parser.module.add_covar_datas(
                self.houseman_parser.module.components,
                covarsnames=self.houseman_parser.module.names,
            )  # add houseman components as covariate file
            if not self.args.gsave:
                logging.info(
                    "To use Houseman estimates as covariates run glint.py with --covarfile %s"
                    % self.houseman_parser.module.outputfile
                )
        ...
        if self.args.ewas:
            ...
            ewas_results = self.ewas_parser.run(
                args=self.args, meth_data=ewas_meth_data
            )

        if self.args.plot:  # if not selected should we plot by default?
            self.plot_parser.run(
                args, meth_data=self.meth_parser.module, ewas_result_obj=ewas_results
            )

        if self.args.epi:
            epi_met_data = self.meth_parser.module.copy()
            self.epi_parser.run(
                args=self.args, meth_data=epi_met_data, output_perfix=self.args.out
            )
            self.meth_parser.module.add_covar_datas(
                self.epi_parser.module.components, "epi"
            )  # add refactor components as covariate file
        ...
        self.meth_parser.save(
            output_perfix=prefix
        )  # save after all preprocessing  add epi and refactor covars
