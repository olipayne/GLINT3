...
def loadtxt(filepath, dtype=None, header=None, delimiter="\t"):
    if dtype:
        x = pd.read_csv(filepath, dtype=dtype, header=header, sep=delimiter)
    else:
        x = pd.read_csv(filepath, dtype=object, header=None, sep=delimiter)
    return x.values


def load_float_data_and_headers(
    filepath, header=None, delimiter="\t", dtype=float32, na_values=None
):
    """
    ...
    sep = get_delimiter(
        second_row
    )  # dont use first row cause it could have only one value(not delimiter)
    if sep != delimiter:
    ...
    first_col = pd.read_csv(
        filepath, dtype=str, delimiter=delimiter, usecols=[0], header=None
    ).values

    first_col = first_col.reshape(
        -1,
    )
    ...
    if len(first_row) != len(second_row):
        ...
        if header == False:
            ...
            terminate(
                "It is assumed that data file doesn't contain an header but it does"
            )
        ...
        data = pd.read_csv(
            ...
            delimiter=delimiter,
            header=0,
            usecols=list(range(1, len(first_row) + 1)),
            na_values=na_values,
        )
        ...
        try:
            ...
            if header or (
                header == None and is_there_a_header(first_col)
            ):  # first option, has the "ID cell"
            ...
            # if there is no header
                ...
                data = pd.read_csv(
                    ...
                    delimiter=delimiter,
                    header=None,
                    usecols=list(range(1, len(first_row))),
                    na_values=na_values,
                )
            ...
            terminate(
                "Error with the format of the file. delimiter must be one of %s and float data"
                % ", ".join(list(DELIMITERS.keys()))
            )
...
def load_data_file(filepath, dim, header=None, dtype=float32, na_values=None):
    ...
    a = time()
    try:
        data, col_names, row_names = load_float_data_and_headers(
            filepath, header, delimiter="\t", dtype=dtype, na_values=na_values
        )
    except Exception as e:
        logging.exception("While loading data")
