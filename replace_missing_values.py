...
def replace_missing(data_filename, missing_value_indicator, data_max_missing_values, samples_max_missing_values, sep = " ", suffix = ".no_missing_values", header = None):
    ...
    try:
        ...
        if all_data.ndim != dim:
            input("Error: got data from dimensions %d while excepted to %d. Please check all paramenters are OK (data type and separator)." %(all_data.ndim, dim))
            return None
        ...
        input("Error loading data file. please check that data_type, separator and missing value indicator Ok\n%s"%e)
