...
def json_numpy_obj_hook(dct):
    ...
    if isinstance(dct, dict) and "__onedarray__" in dct:
        return np.array(dct["__onedarray__"])

    elif isinstance(dct, dict) and "__ndarray__" in dct:
        data = b64decode(dct["__ndarray__"])
        return np.frombuffer(data, dct["dtype"]).reshape(dct["shape"])  # .copy()
    return dct
...
class TitleManager(object):
    ...
    def increase_next_index(self, title_base_name, count):
        ...
        """
        self.title_indexes[title_base_name] = (
            self.title_indexes[title_base_name] + count
        )

    def generate_title(self, title_base_name, count):
        start_i = self.get_next_index(title_base_name)
        titles_list = [
            "%s%d" % (title_base_name, i) for i in range(start_i, start_i + count)
        ]
...
class MethylationData(Module):
    ...
    def __init__(
        ...
        if len(self.samples_ids) != self.samples_size:
            common.terminate(
                "Data contain %s samples but only %s samples ids."
                % (self.samples_size, len(self.samples_ids))
            )

        if len(self.cpgnames) != self.sites_size:
            common.terminate(
                "Data contain %s sites but only %s CpG identifiers."
                % (self.sites_size, len(self.cpgnames))
            )

        logging.debug(
            "Methylation data provided with %s sites and %s samples."
            % (self.sites_size, self.samples_size)
        )
    ...
    def _load_and_validate_file_of_dimentions(self, datafile, dim, header):
        ...
        if not isinstance(datafile, io.TextIOWrapper):
            datafile = open(datafile, "r")

        logging.info("Loading file %s..." % datafile.name)

        data, cols_labels, rows_labels = common.load_data_file(
            datafile.name, dim, header=header
        )
        if data is None:
            common.terminate(
                "There is a problem with the format of the file '%s'." % datafile.name
            )
    ...
    def _validate_samples_ids(self, matrix_sample_ids, samples_ids):
        ...
        """
        if not (
            (samples_ids.size == matrix_sample_ids.size)
            and ((samples_ids == matrix_sample_ids).all())
        ):
            differ = (set(samples_ids)).symmetric_difference(set(matrix_sample_ids))
            if differ:
                if len(differ) > 10:
                    common.terminate(
                        "Sample identifiers are not identical to the sample identifiers in the data file: %s and more."
                        % ", ".join(list(differ)[:10])
                    )
                else:
                    common.terminate(
                        "Sample identifiers are not identical to the sample identifiers in the data file: %s."
                        % ", ".join(list(differ))
                    )
            common.terminate(
                "Sample identifiers are not in the same order as in the data file."
            )

    def _load_and_validate_samples_info(
        self, data_with_samples_info, samples_size, samples_ids
    ):
        """
        ...
        """
        data, header, new_samples_ids = self._load_and_validate_file_of_dimentions(
            data_with_samples_info, 2, None
        )
    ...
    def _load_and_validate_samples_data(
        self, data_list, samples_size, samples_ids, defaule_header_name
    ):
        all_data = []
        all_headers = []

        for datafile in data_list:
            data, header = self._load_and_validate_samples_info(
                datafile, samples_size, samples_ids
            )
            if header is None:
                header = self.header_manager.generate_title(
                    defaule_header_name, data.shape[1]
                )
    ...
    def _load_and_validate_phenotype(
        self,
        phenofile_list,
        samples_size,
        samples_ids,
        default_pheno_name=DEFAULT_PHENO_NAME,
    ):
        """
        ...
        pheno, header = self._load_and_validate_samples_data(
            phenofile_list, samples_size, samples_ids, default_pheno_name
        )
        logging.info("New phenotypes were found: %s." % ", ".join(header))
        return pheno, header

    def _load_and_validate_covar(
        self,
        covarfiles_list,
        samples_size,
        samples_ids,
        default_covar_name=DEAFULT_COVAR_NAME,
    ):
        """
        ...
        covar, header = self._load_and_validate_samples_data(
            covarfiles_list, samples_size, samples_ids, default_covar_name
        )
    ...
    def update_pheno_header(self, titles_to_add):
        if self.phenonames is not None:
            ...
            if mutual_names:
                common.terminate(
                    "More than one phenotype with the name %s exist."
                    % ", ".join(list(mutual_names))
                )
    ...
    def update_covar_header(self, titles_to_add):
        if self.covarnames is not None:
            ...
            if mutual_names:
                common.terminate(
                    "More than one covariate with the name %s exist."
                    % ", ".join(list(mutual_names))
                )
    ...
    def add_covar_files(self, covarfiles_list, default_covar_name=DEAFULT_COVAR_NAME):
        ...
        """
        covars, covarsnames = self._load_and_validate_covar(
            covarfiles_list, self.samples_size, self.samples_ids, default_covar_name
        )
    ...
    def add_pheno_files(self, phenofiles_list, default_pheno_name=DEFAULT_PHENO_NAME):
        ...
        """
        phenos, names = self._load_and_validate_phenotype(
            phenofiles_list, self.samples_size, self.samples_ids, default_pheno_name
        )
        self.update_pheno_data(phenos)
        self.update_pheno_header(names)
        return names

    def upload_new_covaritates_files(
        self, covarfiles_list, default_covar_name=DEAFULT_COVAR_NAME
    ):
        """
        overloads self.covar with the covarfiles_list
        """
        self.covar, self.covarnames = self._load_and_validate_covar(
            covarfiles_list, self.samples_size, self.samples_ids, default_covar_name
        )

    def upload_new_phenotype_file(self, phenofile):
        self.phenotype, self.phenonames = self._load_and_validate_phenotype(
            phenofile, self.samples_size, self.samples_ids
        )
    ...
    def add_covar_datas(
        self, covardata, default_covar_name=DEAFULT_COVAR_NAME, covarsnames=None
    ):
        """
        ...
        if covarsnames is None:
            covarsnames = self.header_manager.generate_title(
                default_covar_name, covardata.shape[1]
            )
    ...
    def exclude_sites_indices(self, sites_indicies_list):
        ...
        if len(sites_indicies_list) == 0:
            ...
            if (
                len(set(range(self.sites_size)).difference(set(sites_indicies_list)))
                == 0
            ):
            ...
            self.sites_size = len(self.cpgnames)
            logging.info(
                "%s sites out of %s were excluded (%s sites left)."
                % (len(sites_indicies_list), size_before, self.sites_size)
            )
    ...
    def remove_samples_indices(self, indices_list):
        ...
        if len(indices_list) == 0:
            ...
            self.samples_size = len(self.samples_ids)
            logging.debug(
                "%s samples out of %s were removed (%s samples left)."
                % (len(indices_list), size_before, self.samples_size)
            )
    ...
    def include(self, include_list):
        ...
        logging.info(
            "Include sites: %s CpGs from the reference list of %s CpGs will be included..."
            % (len(self.cpgnames) - len(remove_indices_list), len(include_list))
        )
        self.exclude_sites_indices(remove_indices_list)
        logging.debug(
            "Methylation data new size is %s sites by %s samples." % self.data.shape
        )
    ...
    def save_sites_and_samples(self, prefix):
        ...
        sites_data = column_stack(
            (
                self.cpgnames,
                sites_info.chromosomes,
                sites_info.positions,
                sites_info.genes,
                sites_info.categories,
            )
        )
        ...
        savetxt(
            sites_filename,
            sites_data,
            fmt="%-12s,\t%-4s,\t%-12s,\t%-22s,\t%-22s",
            header="cpgid, chromosome, position, gene, category",
        )

        samples_filename = prefix + ".samples." + DATA_SUFFIX
        logging.info(
            "Saving sample identifiers and their phenotypes and covariates to %s..."
            % samples_filename
        )
    ...
    def remove_lowest_std_sites(self, lowest_std_th=0.02):
        ...
        sites_std = std(
            self.data, axis=1
        )  # calc variance - missing values are not allowed

        # sort std and get sites index for each std (sorted, so indices of the lowest std sites will be to the left)
        std_sorted_indices = (
            sites_std.argsort()
        )  # sort the sites_variance and return an array that holds the indices of the sorted values
        # get std list sorted
        ...
        if include_from_index == self.sites_size:
            common.terminate(
                "The provided std parameter excluded all sites (%s)." % lowest_std_th
            )
        if include_from_index == 0:
            logging.warning(
                "The provided std parameter excluded no sites (%s)." % lowest_std_th
            )
    ...
    def exclude_maxpcstds(self, pcstds):
        ...
        for pc_index, std_num in pcstds:
            logging.info(
                "Finding samples with standard deviation (STD) higher than %d STDs or lower than %d STDs in principal component number %d..."
                % (std_num, -1 * std_num, pc_index)
            )
            pc = pca_out.P[
                :, pc_index - 1
            ]  # user start counting from 1 and python from 0
            std_pc = std(pc)
            maxpcstds_samples_indices.update(
                where((pc > std_num * std_pc) | (pc < -1 * std_num * std_pc))[0]
            )
    ...
    def get_phenotypes_indicis(self, names_list):
        if self.phenonames is None:  # there are no phenotypes
            common.terminate(
                "There is no phenotype in the data. Add phenotypes file using --phenofile."
            )
        ...
        if len(indices) != len(names_list):
            common.terminate(
                "Used a phenotype name that does not match any of the phenotype names in the data."
            )
        return indices
    ...
    def get_covariates_indicis(self, covariates_names_list):
        if self.covarnames is None:  # there are no covariates
            common.terminate(
                "There is no covariates in the data. Add covariates file using --covarfile."
            )
        assert (
            type(covariates_names_list) == list
            or type(covariates_names_list) == ndarray
        )
        indices = where(in1d(self.covarnames, covariates_names_list))[0]
        if len(indices) != len(covariates_names_list):
            common.terminate(
                "Used a covariate name that does not match any of the covariate names in the data."
            )
        return indices
    ...
    def regress_out(self, matrix_to_regress):
        ...
        ** THIS FUNCTION CHANGES THE METHYLATION DATA **
        ...
        """
        residuals = LinearRegression.regress_out(
            self.data.transpose(), matrix_to_regress
        )
...
class MethylationDataLoader(MethylationData):
    ...
    def __init__(self, datafile, phenofile=[], covarfiles=[]):
        ...
        phenotype, phenonames = self._load_and_validate_phenotype(
            phenofile, samples_size, samples_ids
        )
        covar, covarnames = self._load_and_validate_covar(
            covarfiles, samples_size, samples_ids
        )
        super(MethylationDataLoader, self).__init__(
            data,
            array(samples_ids),
            array(cpgnames),
            phenotype,
            ...
            covarnames,
            phenonames,
            self.header_manager.title_indexes,
        )
    ...
    def _load_and_validate_datafile(self, datafile):
        ...
        """
        data, samples_ids, cpgnames = self._load_and_validate_file_of_dimentions(
            datafile, 2, True
        )

        if cpgnames is None:
            common.terminate(
                "There are no CpG identifiers for the sites in the datafile '%s'."
                % datafile.name
            )
        if samples_ids is None:
            common.terminate(
                "There are no sample identifiers (header) in the datafile '%s'."
                % datafile.name
            )
        logging.info("Checking for missing values in the data file...")
        validate_no_missing_values(
            data
        )  # TODO remove when missing values are supported
